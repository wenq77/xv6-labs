...
mapsuperpages(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa, int perm)
{
  ...
  if((size % SUPERPGSIZE) != 0) {
    printf("size = %p, superpage = %p\n", (void *)size, (void *)(uint64)SUPERPGSIZE);
...
uvmalloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz, int xperm)
{
  ...
  for(a = oldsz; a < newsz; a += sz){
    ...
    if(sz == SUPERPGSIZE && mapsuperpages(pagetable, a, sz, (uint64)mem, PTE_R|PTE_U|xperm) != 0){
      superkfree(mem);
      uvmdealloc(pagetable, a, oldsz);
      return 0;
    }
...
uvmcopy(pagetable_t old, pagetable_t new, uint64 sz)
{
  ...
  for(i = 0; i < sz; i += szinc){
    ...
    pa = PTE2PA(*pte);
    flags = PTE_FLAGS(*pte);
    szinc = *pte & PTE_A ? SUPERPGSIZE: PGSIZE;
    if(szinc == PGSIZE && (mem = kalloc()) == 0)
      goto err;
    if(szinc == SUPERPGSIZE && (mem = superkalloc()) == 0)
      goto err;
    ...
    if(szinc == PGSIZE){
      ...
      if(mapsuperpages(new, i, szinc, (uint64)mem, flags) != 0){
        superkfree(mem);
        goto err;
      }
    }
